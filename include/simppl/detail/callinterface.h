...
struct FunctionCaller
{
   ...
   {
      FunctionCaller<N, TupleT>::eval_intern(f, tuple);
   }
   ...
   {
      FunctionCaller<N, TupleT>::eval_intern_cs(f, cs, tuple);
   }
   ...
   {
     FunctionCaller<N+1 == std::tuple_size<TupleT>::value ? -1 : N+1, TupleT>::eval_intern(f, tuple, t..., std::get<N>(tuple));
   }
   ...
   {
      FunctionCaller<N+1 == std::tuple_size<TupleT>::value ? -1 : N+1, TupleT>::eval_intern_cs(f, cs, tuple, t..., std::get<N>(tuple));
   }
};
...
struct DeserializeAndCall : simppl::NonInstantiable
{
   ...
   void eval(DBusMessageIter& iter, FunctorT& f)
   {
      ...
      FunctionCaller<0, std::tuple<T>>::eval(f, tuple);
   }
   ...
   void evalResponse(DBusMessageIter& iter, FunctorT& f, const simppl::dbus::TCallState<ErrorT>& cs)
   {
      ...
      FunctionCaller<0, std::tuple<T>>::eval_cs(f, cs, tuple);
   }
};
